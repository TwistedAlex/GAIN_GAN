...
def test(cfg, model, device, test_loader, test_dataset, writer, epoch, output_path, batchsize):
    print("******** Test ********")
    model.eval()
    ...
    for epoch in range(chkpnt_epoch, epochs):
        ...
        if epoch == (epochs - 1):
            ...
            test(cfg, model, device, deepfake_psi0_loader.datasets['test'],
                deepfake_psi0_loader.test_dataset, writer, epoch, psi_05_heatmap_path, test_psi05_batchsize)
            ...
            test(cfg, model, device, deepfake_psi1_loader.datasets['test'],
                deepfake_psi1_loader.test_dataset, writer, epoch, psi_1_heatmap_path, test_psi1_batchsize)
