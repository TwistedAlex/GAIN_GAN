...
class batch_GAIN_Deepfake(nn.Module):
    ...
    def forward(self, images, labels, train_flag=False, image_with_masks=None,
        ...
        if train_flag and len(e_masks) > 0:
            ...
            PIL.Image.fromarray(((merged_mask2[0].permute([1, 2, 0])).cpu().detach().numpy() * 255).round().astype(
                np.uint8), 'grayscale').save('/home/shuoli/merged_mask_add.png')
            PIL.Image.fromarray(((merged_mask[0].permute([1, 2, 0])).cpu().detach().numpy() * 255).round().astype(
                np.uint8), 'grayscale').save('/home/shuoli/merged_mask_max.png')
